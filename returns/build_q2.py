...
(not period-specific). Returns/reasons: source/ytd_returns_2.numbers -- the same
rolling returns-app export the Q1 build uses (see build_q1.py).

Run:  python returns/build_q2.py [returns_export] [out.html]
"""
import os
import sys
...
if __name__ == "__main__":
    returns_src = sys.argv[1] if len(sys.argv) > 1 else RETURNS_SRC
    ...
    written = render.render(
        ...
        source_label=f"Matrixify Apr–Jun 2026 (UK+US) + {os.path.basename(returns_src)}",
        out_path=out,
    )
